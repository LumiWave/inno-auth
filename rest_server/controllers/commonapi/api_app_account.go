...
func PostAppAccountLogin(c echo.Context, params *context.Account) error {
	...
	if isExists, isBlocked, err := model.GetDB().VerfiyAccounts(params.InnoUID); !isExists || isBlocked || err != nil {
		if !isExists {
			log.Errorf("RealIP(%v), VerifyAccounts err(%v), inno_uid(%v), isExists(%v)", err, params.InnoUID, isExists)
			resp.SetReturn(resultcode.Result_Auth_Invalid_InnoUID)
		} else if isBlocked {
			log.Errorf("RealIP(%v), VerifyAccounts err(%v), inno_uid(%v), isBlocked(%v)", err, params.InnoUID, isBlocked)
			resp.SetReturn(resultcode.Result_Auth_Blocked_InnoUID)
		} else {
			log.Errorf("RealIP(%v), VerifyAccounts err(%v), inno_uid(%v), isExists(%v), isBlocked(%v)", err, params.InnoUID, isExists, isBlocked)
