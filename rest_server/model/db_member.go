...
func (o *DB) InsertApp(params *context.AppInfo) error {
	sqlQuery := fmt.Sprintf("INSERT INTO onbuff_inno.dbo.auth_app(app_name, cp_idx, login_id, login_pwd, create_dt) output inserted.idx "+
		"VALUES('%v', %v, '%v', '%v', %v)",
		params.AppName, params.CpIdx, params.Account.LoginId, params.Account.LoginPwd, params.CreateDt)
...
func (o *DB) SelectApp(params *context.AppInfo) (*context.AppInfo, error) {
	sqlQuery := fmt.Sprintf("SELECT * FROM onbuff_inno.dbo.auth_app WHERE app_name='%v'", params.AppName)
